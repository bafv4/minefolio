...
実装は `app/content/privacy.md`（プライバシーポリシー）・`app/content/terms.md`（利用規約）の記載に**できる限り準拠**させる。

- 次のいずれかに触れる実装をする前に、ポリシーの記載と矛盾しないか確認する:
  外部サービスへの送信の追加（サーバーからの送信も、埋め込み・CDN・フォント等のブラウザ直接送信も含む）／
  取得する個人情報の追加／Cookie・localStorage の追加や廃止／
  削除・公開範囲・エクスポート等、ユーザーによる管理機能の挙動変更／新しい投稿コンテンツ種別
- **データ最小化**: 機能に不要な個人データを新たに収集しない。ログ・エラーメッセージ・外部送信に
  個人情報（メールアドレス・IPアドレス等）を不用意に含めない
- ポリシーの記載と矛盾する、または記載のないデータの扱いが必要になった場合は、黙って実装せず
  呼び出し元（メイン）に差し戻す。メインはユーザーに確認する
- **ポリシー本文はこのタイミングでは改定しない**（改定はリリース時に `release` スキルでまとめて行う）。
  改定が必要になる変更は、コミット時に `Policy-Revision:` トレーラーでフラグを立てる（`commit` スキル参照）
