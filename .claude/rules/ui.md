...
- トーン:
  ...
  - Shift 同時押し（`needsShift`＝非 shiftHeld 時の単発大文字の逆引き）: 個別 `KeyBadge` 自体に `border-warning/50 bg-warning/10` を付ける（変更なし）。一方 `shiftHeld`（行全体で Shift を押しっぱなし＝「Shiftを押しながら」バリエーション）は、キー列の先頭に独立バッジを置くのではなく `ShiftKeyGroup`（`search-craft-template-view.tsx`）で実入力キー列全体を1つの枠（`rounded-lg border-2 border-warning/50 bg-warning/10 px-2 py-1.5`。ネスト規則どおり内側の `rounded` チップの1段上。余白はリマップリングの外側はみ出し〈約3px〉が枠に触れない値）で囲み、先頭にコンパクトな「⇧」マークのみ（`text-warning`、可視の "Shift" テキストは置かない。`ShiftMark` 内部の sr-only と Tooltip=`playerProfile.withShiftTooltip` が意味を補う）を置く
