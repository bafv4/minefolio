...
- **`/me/keybindings`**: リマップ・カスタムキー・カスタムアクションの登録・更新と、レガシーインポート（`import-legacy`）が不可。既存データは読み取り専用で表示され、該当タブ（およびキー編集モーダルの各セクション）は非活性化される。**キー割り当て（`keybindings`）と指割り当て（`fingerAssignments`）はプリセット未作成でも編集可能**（ユーザー作成時に既定値が投入されるベース設定のため）。
