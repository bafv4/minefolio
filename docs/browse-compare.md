...
- 検索欄内のトグルボタン（VSCode の検索風）で正規表現モードを切り替え。URLパラメータ `?regex=1`
- libSQL に `REGEXP` がないため、`regex=1` のときは LIKE を使わず、フィルタ条件に一致する全件を取得して JS 側で照合する（`new RegExp(q, "i")`、大文字小文字を無視）。照合対象は通常検索と同じく `mcid`・`displayName` のみ（`slug` は含めない）
- 照合・ページングは `loadBrowsePage`（`lib/browse-query.server.ts`）の正規表現分岐で行う（並び順は通常時と同じ＝お気に入り優先＋ソート）
- 不正なパターンは「該当なし」として扱い、クライアント側でも入力中に警告を表示する
